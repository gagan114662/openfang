...
use crate::results::{
    compute_prompt_hash, save_best_prompt, IterationResult, ResultsLogger,
};
...
impl ExperimentRunner {
    pub fn new(config: ExperimentConfig) -> Result<Self, ExperimentError> {
        ...
        let driver = create_driver(&driver_config)
            .map_err(|e| ExperimentError::Driver(format!("{e}")))?;

        let scorer = create_scorer(&config.scoring, Some(Arc::clone(&driver)), &config.model.model)?;
    ...
    pub async fn run(&self) -> Result<ExperimentSummary, ExperimentError> {
        ...
        for iteration in 0..self.config.max_iterations {
            ...
            sentry::add_breadcrumb(sentry::Breadcrumb {
                ...
                message: Some(format!(
                    "iteration {iteration}, prompt_hash={prompt_hash}"
                )),
            ...
            if iteration + 1 < self.config.max_iterations {
                match self
                    .mutator
                    .mutate(&current_prompt, &response_text, score_result.score, iteration)
                    .await
                {
                    Ok(mr) => {
                        if mr.rejected {
...
fn build_experiment_manifest(config: &ExperimentConfig, prompt: &str) -> AgentManifest {
    ...
    manifest.name = format!("experiment-{}", config.name);
...
mod tests {
    ...
    async fn test_experiment_loop_with_mocks() {
        ...
        let runner =
            ExperimentRunner::new_with_deps(config, driver, scorer, mutator).unwrap();
