...
async fn main() -> anyhow::Result<()> {
    ...
    tx.set_data("scoring_strategy", config.scoring_strategy_name().into());
    tx.set_data("mutation_strategy", config.mutation_strategy_name().into());
    tx.set_data(
        "max_iterations",
    ...
    let summary = if cli.dry_run {
        ...
        let scorer = Box::new(MockScorer::new(vec![
            50.0, 65.0, 75.0, 70.0, 80.0, 60.0, 85.0, 72.0, 90.0, 78.0,
        ]));
        let mutator = Box::new(MockMutator::new(vec![
            ...
            format!(
                "{} Reference order numbers when mentioned.",
                config.base_prompt
            ),
        ]));
    ...
    tx.set_data(
        "best_score",
        sentry::protocol::Value::from(summary.best_score),
    );
    tx.set_data(
        "best_iteration",
