...
- Each worker creates and uses a workspace-local `CODEX_HOME` so Codex state, sessions, and snapshots stay inside the issue workspace instead of colliding with the global `~/.codex` path.
